...
<#
  ...
  Use mock flags to simulate failures WITHOUT uninstalling software:
  ...
  Examples:
    ...
    -Mock wrong_java
    -Mock missing_cli
    -Mock missing_profile
    -Mock invalid_profile
    -Mock all
#>
...
function Check-Command {
    ...
    if ($Mock -eq "missing_$cmd" -or $Mock -eq "all") {
        Write-Host "FAIL: $cmd (mocked as missing)"
        return $false
    }
...
if ($Mock -eq "missing_python" -or $Mock -eq "all") {
    Write-Host "FAIL: python not found (mocked)"
...
# Wrong python version mock
if ($Mock -eq "wrong_python") {
    Write-Host "Detected Python major.minor = 3.14 (mocked)"
...
# Mock missing java
if ($Mock -eq "missing_java" -or $Mock -eq "all") {
    Write-Host "FAIL: java not found (mocked)"
...
# Mock wrong java
if ($Mock -eq "wrong_java") {
    Write-Host "Detected java.specification.version = 8 (mocked)"
    Write-Host "FAIL: Java version must be >= 11"
    exit 1
}

# Real Java version check
...
if ($Mock -eq "missing_cli" -or $Mock -eq "all") {
    Write-Host "FAIL: databricks CLI not found (mocked)"
...
if ($Mock -eq "missing_profile" -or $Mock -eq "all") {
    Write-Host "FAIL: No Databricks CLI profile found (mocked)"
    exit 1
}

$answer = Read-Host "Do you already have a Databricks profile configured? (y/n)"

# Mock invalid profile
if ($Mock -eq "invalid_profile") {
    Write-Host "Testing Databricks connectivity..."
    Write-Host "FAIL: Invalid credentials or workspace (mocked)"
    exit 1
}
